...
def vista_llaves_page():
    ...
    # Calcular clasificados con origen de cuadro
    clasificados = []
    clasificados_info = []  # [(nombre, cuadro_num, posicion_en_cuadro)]
    ...
    for cuadro_num in sorted(cuadros.keys()):
        ...
        for pos_idx in range(min(personas_que_pasan, len(jugadores_ordenados))):
            ...
            clasificados_info.append((jugador, cuadro_num, pos_idx + 1))
    
    if len(clasificados) < 2:
        st.warning("⚠️ Se necesitan al menos 2 clasificados. Completa los resultados en los cuadros.")
        return
    
    # Mostrar clasificados con tarjetas premium
    cards_html = f"""
    <div class='dark-section'>
        <h3>🏅 Clasificados — {len(clasificados)} jugadores</h3>
        <div class='clasificados-grid'>
    """
    
    for idx, (nombre, cuadro, pos) in enumerate(clasificados_info):
        seed_class = "seed-gold" if pos == 1 else "seed-silver"
        pos_label = f"{pos}° del Cuadro {cuadro}"
        seed_num = idx + 1
        
        cards_html += f"""
        <div class='clasificado-card'>
            <div class='clasificado-seed {seed_class}'>{seed_num}</div>
            <div class='clasificado-info'>
                <div class='clasificado-name'>{nombre}</div>
                <div class='clasificado-origin'>{pos_label}</div>
            </div>
        </div>
        """
